...
class Dataset_Solar(Dataset):
    ...
    def __read_data__(self):
        ...
        num_valid = int(len(df_numeric) * 0.1)
        border1s = [0, num_train - self.seq_len, len(df_numeric) - num_test - self.seq_len]
        border2s = [num_train, num_train + num_valid, len(df_numeric)]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]

        df_data = df_numeric.values
        ...
        df_stamp = df_date[border1:border2]
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if 'date' in cols:
            cols.remove('date')
        df_date = df_raw[['date']]
        df_numeric = df_raw[cols + [self.target]]
...
class Dataset_Pred(Dataset):
    ...
    def __read_data__(self):
        ...
        border1 = len(df_numeric) - self.seq_len
        border2 = len(df_numeric)
        ...
        tmp_stamp = df_date[border1:border2]
        ...
        self.data_x = data[border1:border2]
        if self.inverse:
            self.data_y = df_numeric.values[border1:border2]
        else:
            self.data_y = data[border1:border2]
        self.data_stamp = data_stamp
...
class generated_time_series_data(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='generated_time_series_data.csv',
                 target='ND', scale=True, timeenc=0, freq='h'):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
