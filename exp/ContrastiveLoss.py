...
class swavloss(torch.nn.Module):
    ...
    def __init__(self, device):
        ...
        self.device = device
        # self.prototypes = torch.nn.Linear(final_out_channels // 2, final_out_channels // 4)
    ...
    def forward(self, t_vec, f_vec):
        assign_t = torch.nn.functional.normalize(t_vec, dim=1, p=2)  ##add after T encoder
        assign_f = torch.nn.functional.normalize(f_vec, dim=1, p=2)  ##add after F encoder
        ...
        swavloss = -20 * torch.mean(q_t * torch.log(p_f) + q_f * torch.log(p_t))
        swavloss.requires_grad_()
